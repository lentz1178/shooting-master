...
def draw_menu():
    global game_over, pause, mode, level, menu, time_passed, ammo, total_shots, points, time_remaining, best_freeplay, best_ammo, best_timed, write_values, new_coords
    game_over = False
    pause = False
...
def draw_game_over():
    global level, pause, menu, points, total_shots, time_passed, time_remaining, clicked, game_over, menu_button
    if mode == 0:
    ...
    mouse_pos = pygame.mouse.get_pos()
    clicks = pygame.mouse.get_pressed()
    exit_button = pygame.draw.rect(screen, 'green', [470, 644, 260, 100], 3)
    resume_button = pygame.draw.rect(screen, 'green', [170, 661, 260, 100], 3)
    menu_button = pygame.draw.rect(screen, 'green', [470, 661, 260, 100], 3)
    screen.blit(font.render(f'Score: {display_score}', True, 'black'), (650, 570))
...
while running:
    ...
    if new_coords:
        ...
        for i in range(3):
            my_list = targets[2]
            for j in range(my_list[2][i]):
                two_coords[i].append((WIDTH // (targets[2][i]) * j, 300 - (i * 150) + 30 * (j % 2)))
        for i in range(4):
            my_list = targets[3]
            for j in range(my_list[i]):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if (0 < mouse_position[0] < WIDTH) and (0 < mouse_position[1] < HEIGHT - 200):
                ...
                if mode == 1:
                    ammo -= 1
            #pause button location on game mode  
            if (670 < mouse_position[0] < 860) and (610 < mouse_position[1] <715):
                resume_level = level
                pause = True
                clicked = True
            #reset buttion location right below the pause button
            if (670 < mouse_position[0] < 860) and (725 < mouse_position[1] < 760):
                menu = True
                clicked = True  
